    ...
    <div className="wrap">
      <header className="top">
        ...
        <div className="status">
          ...
          <Badge>{ping?.model ? `Model: ${ping.model}` : "No model set"}</Badge>
          <Badge>{ping?.rules_source ? `Rules: ${ping.rules_source}` : ""}</Badge>
        </div>
      </header>
      ...
      >
        ...
        <p className="hint">Tip: Press <kbd>Ctrl</kbd> + <kbd>Enter</kbd> to run.</p>
      </SectionCard>
      ...
      {result && (
        <>
          ...
          >
            <div className="kv">
              <div><strong>Detected Error:</strong> {result.detected_error}</div>
              {Array.isArray(result.signals) && result.signals.length > 0 && (
                <div><strong>Signals:</strong> {result.signals.join(", ")}</div>
              )}
            </div>
          </SectionCard>
          ...
          <div className="columns">
            ...
            <SectionCard
              ...
              extra={
                <button
                  className="btn subtle"
                  onClick={() => copy((result.references || []).join("\n"))}
                >
                  Copy
                </button>
              }
